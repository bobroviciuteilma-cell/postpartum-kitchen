...
steps → finish; timers; Back everywhere; no-cache meta; **no dial — vessel-bound**, per the ratio
law) and **NEW** `<slug>-mum.html` = Mum's card (why + 3-label chips → real tappable Sources sheet +
"Your instructions" + Send to my helper). Existing URLs unchanged. Generated by `cardgen/gen_drinks.py`
from `cardgen/data_*.py`; per-card QA = `cardgen/qa_drinks.py` (30/30 clean). Images moved to shared
`cook-cards/img/<slug>/` (Decision 0 → shared folder; cards ~10× smaller; the photo pass becomes a
file swap). `drinks.html` now offers **Your card / Helper's kitchen card** per recipe (`?v=c1`).

Claims corrections carried from TAXONOMY-SYNERGY.md (A.1/A.2):
- **Fennel Tea** — the "cups, not litres" dose note added (LactMed NBK501793).
- **Ashwagandha Milk** — rewritten to the honest judgment-call copy (no "safe"/no "dangerous";
  culinary pinch ≠ extract; thyroid + bed-sharing flags; LactMed + e-lactancia + thyroiditis sources).
  The same honest note backs the golden-milk evening boost.
- **Adrenal Cocktail → "Mineral Refresher"** (file name unchanged) — electrolytes + vitamin C framing;
  the rename is explained ON the card with the debunk sources (PMC4997656, Endocrine Society).
- **Golden Milk** — "~20×" multiplier dropped (mechanism kept, PMC5358025); cinnamon blood-sugar claim
  removed; collagen = "building blocks", not outcomes.
- **Meat Stock Latte** — broth reframed collagen/glycine + comfort (Springer/PMC5533136); egg-yolk
  choline is the star (ODS Choline); freshest-eggs warning inline at the blend step.
- **Beet Kvass** — leads with folate; non-iodised salt + beets-under-brine + trust-your-nose carried
  as contextual step warnings (BCCDC); day-chips + strain step; "Jar started — tell Mum" finish.
- **Power Smoothie** — plant omega-3 named **ALA** honestly (Harvard flax); DHA story stays with fish cards.
- **Masala Chai** — caffeine BF note per LactMed NBK501467 (~300 mg/day; fetched & verified 2026-07-07).
- **Jujube & Goji** — tea-iron softened to tradition; confinement-brew heritage labelled tradition.

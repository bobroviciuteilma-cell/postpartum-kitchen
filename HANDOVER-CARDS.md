...
- **SOURCE ATTRIBUTION (locked) — every card must say where the recipe came from + what you boosted.** The `.lineage` "Why we say this" note LEADS with provenance so Ilma can see at a glance *what's original, what's sourced, what's boosted*. Rules:
  ...
  - **Source of truth for links = `index.html`** subtitles (each recipe's `<div class="subtitle">` has the @handle + URL). Known corrections Ilma has given override it: **Chicken Liver Pâté = @sokoladassielai** (Lithuanian — Rūta Banionytė / "Šokoladas Sielai"; note the handle ends `-ai`, earlier mis-recorded as `@sokoladassiela`), **exact recipe = https://www.instagram.com/p/DK2QN9EtwyT/** (verified via Apify 2026-07-02); the Pâté **Pops** freeze-idea = @wildnutritionist at https://www.instagram.com/p/DY52esFAUHk/ (index.html wrongly credited the whole pâté to @wildnutritionist). When Ilma pastes new source links, add them here and to the card.

...
3. **Swap the 4 gated "My version" cards** for the real recipes once owner fetches them.
   ...
   - **PP GOLD VERSION ✦ (reusable, 2026-06-21):** a recipe = a **standard** version + an optional **postpartum/
     confinement upgrade**. The PP-only ingredients go under a gold `<div class="subgroup goldgroup">`
     `<span class="goldmark">&#10022;</span> GOLDEN BOOST</div>` header (display text "✦ PP GOLD VERSION"; method bits get inline `.boostline` "✦ PP Gold" notes — NOT a separate callout box, that read repetitive). Older `.box ppboost`
     ...
     "PP Gold" — Ilma: "stick to PP Gold", do NOT call it "Golden Boost". Reuse this pattern on any recipe with a confinement upgrade.
